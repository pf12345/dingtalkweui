<template>
  <div class="dingtalkweui-slider">
    <div class="dingtalkweui-swiper" :style="{height: xheight}">
      <slot></slot>

      <div
      class="dingtalkweui-swiper-item"
      v-for="(item, index) in list"
      @click="clickListItem(item)"
      :data-index="index">
        <a href="javascript:">
          <div class="dingtalkweui-img" :style="{backgroundImage: buildBackgroundUrl(item)}"></div>
          <p class="dingtalkweui-swiper-desc" v-if="showDescMask">{{ item.title }}</p>
        </a>
      </div>

      <div
      v-if="listTwoLoopItem.length > 0"
      class="dingtalkweui-swiper-item dingtalkweui-swiper-item-clone"
    ...
    <div :class="[dotsClass, 'dingtalkweui-indicator', `dingtalkweui-indicator-${dotsPosition}`]" v-show="showDots">
      <a href="javascript:" v-for="key in length">
        <i class="dingtalkweui-icon-dot" :class="{'active': key - 1 === current}"></i>
      </a>
    </div>
  </div>
</template>
